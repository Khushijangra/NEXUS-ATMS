...
def _setup_gpu(preferred_device: str = "cuda") -> str:
    """Configure SB3 DQN device with CUDA fallback handling."""
    preferred_device = (preferred_device or "cuda").lower()
    if preferred_device == "cpu":
        print("[SB3] DQN device set to CPU")
        return "cpu"
    if not torch.cuda.is_available():
        print("[SB3] CUDA not available for DQN - using CPU")
        return "cpu"
    gpu_name = torch.cuda.get_device_name(0)
    print(f"[SB3] DQN using CUDA on {gpu_name}")
    return "cuda"

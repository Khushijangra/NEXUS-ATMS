...
MODULES_ANOMALY = PROJECT_ROOT / "modules" / "anomaly"
if str(MODULES_ANOMALY) not in sys.path:
    sys.path.append(str(MODULES_ANOMALY))

try:
    from src.models.backbones.videomae import VideoMAEFeatureExtractor
except ImportError:
    logger.error(f"Cannot import VideoMAEFeatureExtractor. Make sure {MODULES_ANOMALY} exists and contains src.models.backbones.videomae.")
    sys.exit(1)

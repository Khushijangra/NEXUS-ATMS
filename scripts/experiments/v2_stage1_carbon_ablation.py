...
class NumpyARGUSEngine:
    ...
    def process_frame(self):
        if self.idx >= len(self.features): self.idx = 0  
...
def run_carbon_ablation():
    ...
    for w in weights:
        reward_type = "carbon_combined" if w > 0 else "combined"
        print(f"\\n--- Evaluating weight={w}, reward_type={reward_type} ---")
        ...
        for seed in seeds:
            ...
            log_dir = str(project_root / "outputs" / f"ppo_ablation_{w}_{seed}")
            model_dir = str(project_root / "outputs" / f"ppo_ablation_models_{w}_{seed}")
            agent = PPOAgent(env=env, config=config, log_dir=log_dir, model_dir=model_dir)
            
            # Train for 10000 timesteps as requested
            ...
            results.append({
                "weight": w,
                "seed": seed,
                "reward": final_metrics.get("total_reward", 0.0),
                "queue": final_metrics.get("total_queue_length", 0.0),
                "waiting": final_metrics.get("total_waiting_time", 0.0),
                "throughput": final_metrics.get("throughput", 0),
                "fuel": final_metrics.get("fuel_l", 0.0),
                "co2": final_metrics.get("co2_kg", 0.0),
                "wait_reward": final_metrics.get("wait_reward", 0.0),
                "queue_reward": final_metrics.get("queue_reward", 0.0),
                "throughput_reward": final_metrics.get("throughput_reward", 0.0),
                "carbon_reward": final_metrics.get("carbon_reward", 0.0),
                "variance": np.var(rewards_list),
                "policy_loss": 0.0, # Placeholder unless we parse SB3 logs
                "value_loss": 0.0,
                "entropy": 0.0
            })
    ...
    # Save to CSV
    csv_path = out_dir / "carbon_ablation.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "weight", "seed", "reward", "queue", "waiting", 
            "throughput", "fuel", "co2", "wait_reward",
            "queue_reward", "throughput_reward", "carbon_reward",
            "variance", "policy_loss", "value_loss", "entropy"
        ])
        writer.writeheader()

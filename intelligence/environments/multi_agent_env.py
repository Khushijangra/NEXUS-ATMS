...
from intelligence.orchestration.graph_coordinator import GraphCoordinator
from intelligence.orchestration.graph_state_builder import GraphStateBuilder
...
class MultiAgentSumoEnv(gym.Env):
    ...
    def __init__(
        ...
        self.shared_policy_mode = bool(shared_policy_mode)
        ...
        # Graph coordination modules (default-off for safe A/B testing).
        self._graph_builder = GraphStateBuilder(debug=self.graph_debug)
        self._graph_coordinator = GraphCoordinator(debug=self.graph_debug)

        self._base_obs_dim = self.N_JUNCTIONS * self.LOCAL_STATE_DIM
        self._obs_dim = self._base_obs_dim * (2 if self.graph_enabled else 1)
        ...
        if self.shared_policy_mode:
            self.action_space = spaces.Discrete(self.N_ACTIONS_PER_JUNCTION)
        else:
    ...
    def _get_full_state(self) -> np.ndarray:
        """Build flattened observation for all 16 junctions.

        Graph OFF: concat(local_state_i)
        Graph ON : concat(concat(local_state_i, mean(neighbor_state_i)))
        """
        ...
        local_matrix = np.stack(states, axis=0).astype(np.float32)
        if not self.graph_enabled:
            return local_matrix.reshape(-1)

        adjacency = self.get_adjacency_matrix()
        enhanced = self._graph_coordinator.enhance(local_matrix, adjacency)
        if self.graph_debug:
            logger.info(
                "[MultiAgentSumoEnv] graph enabled local=%s enhanced=%s",
                tuple(local_matrix.shape),
                tuple(enhanced.shape),
            )
        return enhanced.reshape(-1)

    def get_neighbor_mapping(self) -> Dict[str, Dict[str, Optional[str]]]:
        """Return cardinal neighbor mapping for each junction."""
        out: Dict[str, Dict[str, Optional[str]]] = {}
        for jid, nbs in self._junction_neighbors.items():
            out[jid] = {
                "north": nbs[0],
                "south": nbs[1],
                "west": nbs[2],
    ...
    def get_adjacency_matrix(self) -> np.ndarray:
        """Return adjacency matrix aligned with `self.junction_ids`."""
        n = len(self.junction_ids)
        id_to_idx = {jid: i for i, jid in enumerate(self.junction_ids)}
        adj = np.zeros((n, n), dtype=np.float32)

        for jid, mapping in self.get_neighbor_mapping().items():
            i = id_to_idx[jid]
            for nb in mapping.values():
                if not nb:
                    continue
                j = id_to_idx.get(nb)
                if j is None:
    ...
    def get_graph_snapshot(self) -> Dict[str, np.ndarray]:
        """Build current graph snapshot: node features + adjacency + enhanced features."""
        sim_time = traci.simulation.getTime()
        total_vehicles = max(traci.vehicle.getIDCount(), 1)
        halted = sum(
            1 for vid in traci.vehicle.getIDList()
        ...
        local_states = [self._get_junction_state(jid, global_congestion, sim_time) for jid in self.junction_ids]
        node_features = np.stack(local_states, axis=0).astype(np.float32)
        adjacency = self.get_adjacency_matrix()
        ...
        return {
            ...
            "enhanced_node_features": enhanced,
        }
    ...
    def step(self, action):
        ...
        if self.shared_policy_mode:
            ...
            action = np.full((self.N_JUNCTIONS,), action_value, dtype=np.int64)
        ...
        info["graph_enabled"] = self.graph_enabled
        info["shared_policy_mode"] = self.shared_policy_mode
        if terminated:

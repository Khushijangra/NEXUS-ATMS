    ...
    def _init_yolo(self) -> None:
        try:
            ...
            try:
                ...
                logger.warning(
                    "[Detector] Could not use requested device '%s' (%s). Falling back to CPU.",
                    self.device,
                    exc,
                )
                self.device = "cpu"
                self._model.to(self.device)
                logger.info("[Detector] YOLOv8n loaded on cpu.")
        except ImportError:
            logger.warning(
        ...
        try:
            ...
            if not proto.exists() or not model.exists():
                logger.info(
                    ...
                    "  Download MobileNetSSD prototxt + caffemodel into ai/vision/weights/."
                )
